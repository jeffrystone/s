...
## Воспроизводимость RNG (`simulation_rng_seed`)

- При **`Settings.use_simulation_rng_seed == true`** и заданном **`simulation_rng_seed::UInt64`** функция **`build_environment`** переводит внутреннюю сборку узлов на поток **`Random.Xoshiro(...)`**, **игнорируя** аргумент ключевого слова **`rng`** при генерации параметров узлов. Это фиксирует стартовую популяцию при повторном вызове с тем же `simulation_rng_seed`.
- После сборки симуляцию по-прежнему задаёт RNG, переданный в **`step!` / `simulate!`**. Для воспроизводимости полного траекторного прогона используйте **один явный объект RNG** последовательно (например создать **`Random.Xoshiro(seed_outer)`**, передать его в **`build_environment`**, когда **не** включён режим симуляционного семени, затем использовать **тот же** экземпляр в каждой итерации **`step!`**).
- Если включены **`parallel_offspring`** и достаточно попыток оффспринга так, что включается **`Threads.@threads`**, возможен недетерминизм между запусками (побочное изменение локальных генераторов). Для деталей при **`evaluate_cache_enabled == true`** параллельный оффспринг при **`parallel_offspring_disable_with_eval_cache`** может отключаться; см. код **`perform_resonance_between!`**.
- Опционально **`incremental_D_pollard`**: экономит полный **`dot(metric_weights, metric_components)`** только по пути успешной Pollard-съёмки веса уровня; при изменении **`env.metric_weights`** на том же такте см. апелляции — они вызывают полный **`recompute_D!`**.
...
Удалить узел по `node_id`. Если **`Settings.manual_delete_node_creates_scar == true`** (по умолчанию так), перед удалением в `env.scars` добавляется запись через **`failure_scar_meta`** параметров узла (`potential = 1.0`; радиус, `fail_level` и затухание определяет задача), чтобы снизить риск немедленного повторного появления того же паттерна.
...
Включить или выключить участие узла как инициатора обычных выстрелов/тяжёлого выстрела в планировщике (поле узла **`active`**; при `false` узел не добавляет своих SHOT в очередь, но может оставаться в популяции с `hp`/`mp`).

```json
{"action":"set_active","node_id":"1","active":false}
```
...
Размер буфера калибровки L3/L4 настраивается в `Settings` (`analysis_calibration_*`). Калибровка запускается на проходе ANALYSIS при достаточном числе наблюдений пар нормализованных показателей после выстрелов L4. **Псевдо-популяционная выборка:** узлы, у которых уже заполнены слоты `metric_components[3]` и `[4]` (без нового `evaluate`), могут добавить до `analysis_population_calibration_pairs_max` пар в буфер через `maybe_push_population_l34_pairs!` — это усиливает статистику весов, не являясь полным обходом популяции с дорогими оценками.
